...
The WideTrellis is UltraDim's index. Two things about it matter to a user.
A family becomes searchable after one settle; after that, every row you
insert is searchable at once, and the index keeps improving behind the
arrivals, so there is never a window in which ingested data is unavailable.
And every score you get back is the exact cosine against the stored vector,
which you can verify against exact ground truth on demand
(`trellis_measure_recall`). Builds are deterministic: same rows, same seed,
same index. Replaced and deleted rows never require a rebuild.
...
is continuous: each new record is searchable at first touch, and the index keeps improving
behind the arrivals.
...
| `trellis_template_status` | Build progress and whether search can serve |
...
| `cluster` | Spherical k-means over the reduced rows — **sparse only** |
...
### Why a settle can refuse

`trellis_template_settle` refuses with "settle produced an EMPTY graph" when
no pair of rows in the corpus is similar enough, rather than building an
index of noise.

That refusal is almost always the corpus, not the engine. A few hundred rows
of uniformly random sparse vectors have no real neighbours: draw 32 non-zeros
from 100,000 dimensions and nearly every pair shares no dimension at all, so
the true top-10 is a tie at zero. Give the family real structure before
reaching for the `floor` override. The test harness plants 20 topic groups
whose same-topic pairs have a median cosine around 0.5, and settles at the
shipped default with no override at all.
...
`make_searchable` resolves the field file and settles the index itself, so you
never touch `field_path` or `floor`. Actual output from the run above:
...
isolated. A real cache with more near-duplicates has more neighbours; nothing here is
a workaround.
...
corpus is sparse enough that its real neighbours sit below the default
floor (0.3314 at width 128) and the settle refuses with an empty graph, fall
...
- **`make_searchable(name)` is the one-call way in.** It drives
  `BuildUltradimV23TrellisIndex`, which resolves the field file and settles the
  index in one call, so the common case needs no `field_path` and no `floor`. The raw `trellis_template_settle` stays for the one case
...
- **Tool descriptions carry their one gotcha inline** — settle's `field_path`,
  ...
  tool's floor text was corrected: the default is 0.3314 at width 128, not
  the 0.53 it used to claim.
