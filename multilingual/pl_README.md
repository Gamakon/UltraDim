...
<p align="center"><i>Przestrzeń chemiczna jako BloomMap: 50 000 cząsteczek ChEMBL, pogrupowanych hierarchicznie przy ich natywnych 30 000 000 wymiarach.</i></p>
...
- **Dokładne uporządkowanie najbliższych sąsiadów** — nasze dwustopniowe wyszukiwanie znajduje kandydatów i porządkuje ich dokładnie za pomocą obliczenia prawdziwego kosinusa.
- **Wbudowane funkcje do audytu pełności (recall) i opóźnienia indeksu** — wraz z funkcją automatycznego strojenia, która przeszukuje ustawienia.
...
**Przeszukuj ultraszerokie dane i ufaj odpowiedziom.** UltraDim obsługuje Twoje dane przy ich natywnej wymiarowości — odciski molekularne, koszyki zakupowe, genomikę w kodowaniu one-hot, osadzenia tekstu — bez haszowania cech ani obcinania po Twojej stronie. Wyniki, które otrzymujesz, to prawdziwe kosinusy względem Twoich surowych wektorów, a indeks mierzy na żądanie własną pełność kandydatów względem dokładnych odpowiedzi z przeszukiwania wyczerpującego, więc każdy korpus otrzymuje certyfikat jakości pełności.

**Buduj silniki rekomendacyjne działające w czasie rzeczywistym.** Zdolność UltraDim do wyznaczania prawdziwych najbliższych sąsiadów w ogromnej liczbie wymiarów otwiera zupełnie nowe sposoby budowania silników rekomendacyjnych: możemy odpytywać bazę danych wprost o osoby, które kupowały podobnie do Ciebie, i na tej podstawie tworzyć rekomendację zawsze aktualną. Nasze testy na MovieLens-20M dają Recall@20 = 0,362 bez żadnych modeli. Połączenie naszych wyników wyszukiwania z dekoderem grzbietowym podniosło tę wartość do 0,3900 (przedział ufności 95 % [0,3847; 0,3955]) przy NDCG@100 = 0,4229 i było obsługiwane w około 17 ms na użytkownika na laptopie Mac. Rekomendacje podążają za zmieniającym się zachowaniem użytkowników w czasie rzeczywistym.

**Buduj żywe mapy 2-D danych o 30 M wymiarów.** Mapy UMAP są obliczane natywnie na serwerze, więc budowanie map działa z dużymi wektorami o ekstremalnej długości, przy których inne narzędzia z trudem w ogóle ładują dane. Mapy są żywymi obiektami: nowe punkty są umieszczane na istniejącej mapie w milisekundach, małe partie wtapiają się za około 1,5 % kosztu przebudowy, a historia wersji mapy służy zarazem jako detektor dryfu Twojego strumienia danych.
...
**Grupuj i oceniaj wektory dowolnej długości.** Sferyczne k-średnich i grupowanie hierarchiczne działają rezydentnie na GPU przy ekstremalnej wymiarowości, z miarami jakości skorygowanymi o wymiar, które pozostają porównywalne między wymiarowościami; dzięki temu pytanie „czy to grupowanie jest rzeczywiste?” ma statystyczną odpowiedź przy 30 M wymiarów, a nie tylko przy 300.
