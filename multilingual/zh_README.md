...
UltraDim 存储、搜索、映射、分类和聚类向量，其维度远远超出传统向量数据库的限制：稠密数据可达约 256,000 维，稀疏数据可达数千万维，并已在真实化学语料上以 **30,000,000 维**的生产运行得到验证。它以 Rust 编写，通过 macOS 上的 Metal 和 Linux 上的 Vulkan 实现 GPU 加速，并由 Python 驱动。
...
- **原生宽度摄入** — 稠密向量可达约 256,000 维；稀疏向量可达数千万维（已验证 3000 万维），内存随非零元素数量增长，而非随声明的宽度增长
