...
**搜索超宽数据，并信任其结果。** UltraDim 以数据的原生维度处理您的数据——分子指纹、零售购物篮、one-hot 编码的基因组数据、文本嵌入——您无需做特征哈希或截断。您收到的分数是针对原始向量计算的真余弦值，且索引可按需将自身的候选召回率与精确暴力搜索的答案进行对照测量，因此每个语料库都附带一份召回率的质量证书。

**构建实时推荐引擎。** UltraDim 在超大维度上找出真正最近邻的能力，为构建推荐引擎提供了全新的途径：我们可以直接向数据库查询"与您购买行为相似的人"，并据此给出始终保持最新的推荐。我们在 MovieLens-20M 上的测试表明，在不使用任何模型的情况下 Recall@20 = 0.362。将我们的搜索结果与一个岭回归解码器融合后，该指标提升至 0.3900（95% 置信区间 [0.3847, 0.3955]），NDCG@100 = 0.4229，且在一台 Mac 笔记本电脑上每位用户的服务耗时约为 17 ms。推荐结果随用户行为的变化实时漂移。
